...
public interface SynopticRepository extends JpaRepository<Synoptic, Long> {
    ...
    @Query(value = "select s.pressure from synoptic s where s.pressure is not null",
            nativeQuery = true)
    List<Double> findByPressure();

    @Query(value = "select s.wind_speed from synoptic s where s.wind_speed is not null",
            nativeQuery = true)
    List<Double> findByWindSpeed();

    @Query(value = "select * from synoptic s where s.temperature is not null and s.station is not null",
            nativeQuery = true)
